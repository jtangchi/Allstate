'''
Implementation of xgboost in prediction of Allstate claims severity. 
'''
...
def encode(charcode):
    ...
    '''
    focusing the interactions within the categorical features. 
    '''
    r = 0
    ln = len(str(charcode))
    for i in range(ln):
...
def fair_obj(preds, dtrain):
    '''
    Customized the objective function for the xgboost model.
    The mean absolute error is not differential.
    '''
...
def mungeskewed(train, test, numeric_feats):
    ...
    '''
    fixing the features bias by removing those features with high skewness. 
    '''
...
if __name__ == "__main__":
    ...
    for i, (train_index, test_index) in enumerate(kf):
        ...
        # parameters are tuned by bayes optimization and hyopt. 
